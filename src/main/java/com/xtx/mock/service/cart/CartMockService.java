...
public class CartMockService {
    ...
    public JSONArray queryCartList(String uid, JSONObject masterSkus) {
        ...
        for (Map.Entry<String, JSONObject> entry : store.getAllCarts().entrySet()) {
            ...
            int availableStock = sku != null ? MockStockUtils.availableStock(sku) : cart.getInt("stock", 0);
            item.set("stock", availableStock);
            item.set("availableStock", availableStock);
            item.set("stockStatus", sku != null ? MockStockUtils.stockStatus(sku) : MockStockUtils.stockStatus(cart));
            item.set("count", cart.getInt("count", 1));
            item.set("isEffective", cart.getBool("isEffective", true) && availableStock > 0);
    ...
    public String addToCart(String uid, String skuId, int count,
        ...
        if (existing != null) {
            ...
            JSONObject sku = masterSkus != null ? masterSkus.getJSONObject(skuId) : null;
            int availableStock = MockStockUtils.availableStock(sku);
            int targetCount = cart.getInt("count", 0) + count;
            if (targetCount > availableStock) {
                return "库存不足";
            }
            cart.set("count", targetCount);
            cart.set("stock", availableStock);
            store.save();
            return null;
        }
        ...
        cartItem.set("stock", availableStock);
        cartItem.set("count", count);
        cartItem.set("isEffective", availableStock > 0);
     ...
     */
    public boolean updateCart(String uid, String skuId, Map<String, Object> params, JSONObject masterSkus) {
        ...
        cart.set("stock", availableStock);
        cart.set("isEffective", availableStock > 0);
        store.save();
        return true;
    }
        ...
        if (cartList != null) {
            for (Map<String, Object> item : cartList) {
                ...
                if (existing != null) {
                    ...
                    newItem.set("stock", MockStockUtils.availableStock(sku));
                    newItem.set("count", count);
                    newItem.set("isEffective", MockStockUtils.availableStock(sku) > 0);
