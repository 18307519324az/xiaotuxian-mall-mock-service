...
import static com.xtx.mock.service.order.StockChangeLogService.CHANGE_TYPE_LOCK;
    ...
    public JSONObject createOrder(String userId, Map<String, Object> params) {
        ...
        if (requestGoodsList != null) {
            ...
            for (Map<String, Object> g : requestGoodsList) {
                ...
                if (count > MockStockUtils.availableStock(sku)) {
                    throw new IllegalArgumentException("库存不足: " + skuId);
                }
                ...
                cartItem.set("stock", MockStockUtils.availableStock(sku));
                cartItem.set("isEffective", MockStockUtils.availableStock(sku) > 0);
        ...
        for (Map.Entry<String, JSONObject> entry : selectedEntries) {
            ...
            JSONObject skuForStock = masterSkus.getJSONObject(skuId);
            if (skuForStock == null) {
                throw new IllegalArgumentException("商品不存在: " + skuId);
            }
            if (count > MockStockUtils.availableStock(skuForStock)) {
                throw new IllegalArgumentException("库存不足: " + skuId);
            }
            JSONObject skuForPrice = skuForStock;
            String priceStr;
            if (skuForPrice != null) {
            ...
            skuItem.set("availableStock", MockStockUtils.availableStock(skuForPrice));
            skuItem.set("stockStatus", MockStockUtils.stockStatus(skuForPrice));
        ...
        // ── Step 10a: 锁定库存（availableStock → lockedStock）──
        for (Object obj : orderSkus) {
            JSONObject skuItem = (JSONObject) obj;
            int count = skuItem.getInt("count", 1);
            JSONObject sku = masterSkus.getJSONObject(skuItem.getStr("skuId"));
            ...
            int newAvail = Math.max(0, beforeAvail - count);
            int newLocked = beforeLocked + count;
            ...
            stockChangeLogService.record(
                    orderNo, skuItem.getStr("skuId"), skuItem.getStr("goodsId"),
                    CHANGE_TYPE_LOCK, -count,
                    beforeAvail, newAvail,
                    beforeLocked, newLocked,
                    beforeSold, beforeSold);

            JSONObject product = masterProducts.getJSONObject(skuItem.getStr("goodsId"));
            if (product != null) {
                int pBeforeAvail = product.getInt("availableStock", product.getInt("inventory", 0));
                int pBeforeLocked = product.getInt("lockedStock", 0);
                int pNewAvail = Math.max(0, pBeforeAvail - count);
                int pNewLocked = pBeforeLocked + count;
